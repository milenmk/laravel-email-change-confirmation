...
class EmailChangeConfirmationServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        $this->mergeConfigFrom(
            __DIR__.'/../config/email-change-confirmation.php',
    ...
    protected function validateSecurityConfiguration(): void
    {
        ...
        if ($expireMinutes > 1440) { // 24 hours
            \Log::warning('Email change confirmation expiration is set to more than 24 hours, which may pose security risks', [
        ...
        if (!$hashSecret) {
            \Log::warning('EMAIL_CHANGE_HASH_SECRET not configured - email hashes will use weaker security. Set EMAIL_CHANGE_HASH_SECRET in your .env file.');
        ...
        if ($hashSecret && $this->isWeakHashSecret($hashSecret)) {
            \Log::warning('EMAIL_CHANGE_HASH_SECRET appears to be weak or predictable. Use a cryptographically secure random string.');
        }
